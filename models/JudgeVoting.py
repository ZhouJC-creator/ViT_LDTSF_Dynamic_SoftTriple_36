...
def inverse_transform_sampling(weights, num_samples):
    """
    使用CDF的逆函数进行确定性采样，从归一化后的权重分布中采样 token 索引。
    参数：
        weights: Tensor，形状 [B, L]，每个 token 的得分
        num_samples: 每个样本采样的 token 数量
    返回：
    ...
    """
    # 归一化得分为概率分布
    probs = torch.softmax(weights, dim=-1)  # [B, L]
    # 计算累积分布函数（CDF）
    cdf = torch.cumsum(probs, dim=-1)  # [B, L]

    # 生成确定性分位点 (inverse CDF 的输入)
    step = 1.0 / (2 * num_samples)
    quantiles = torch.linspace(step, 1 - step, num_samples, device=weights.device)  # [num_samples]
    quantiles = quantiles.unsqueeze(0).expand(probs.size(0), -1)  # [B, num_samples]

    # 通过 searchsorted 找到满足 F(x) >= q 的最小索引
    indices = torch.searchsorted(cdf, quantiles, right=True)  # [B, num_samples]
    indices = torch.clamp(indices, max=weights.size(1) - 1)  # 避免越界
...
class DynamicSelect(nn.Module):
    ...
    def forward(self, attn_weights, hidden_states, select_num=None, last=False):
        ...
        # (32, 784)
        count_sum = torch.zeros((B, 784), dtype=torch.int, device='cuda').half()
        ...
        for i, weight in enumerate(attn_weights):
            ...
            for b in range(B):
                ...
                token_importance = weight[:, :, 0, 1:].mean(dim=1)
                selected_inx = inverse_transform_sampling(token_importance, select_num[0][i])
        ...
        for i, item_inx in enumerate(selected_inx_all):
            scores = torch.bincount(item_inx, minlength=784)
            count_sum[i, :] += scores
